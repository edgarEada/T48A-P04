# -*- coding: utf-8 -*-
"""Cuadros y gráficos estadísticos   Error cuadrático medio 178361.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1vuDRjdQRgqyU75XyBw3I70MAO4THYvKT
...
# AFC East
afc_east_pct = [0.688, 0.688, 0.563, 0.250, 0.125]

# AFC Central
afc_central_pct = [0.688, 0.625, 0.438, 0.313]

# AFC West
afc_west_pct = [0.688, 0.625, 0.500, 0.438, 0.125]

# NFC East
nfc_east_pct = [0.813, 0.688, 0.563, 0.375, 0.250]

# NFC Central
nfc_central_pct = [0.688, 0.563, 0.313, 0.313, 0.313]

# NFC West
nfc_west_pct = [0.875, 0.750, 0.375, 0.375]

total_pct = np.array(afc_east_pct + afc_central_pct + afc_west_pct + nfc_east_pct + nfc_central_pct + nfc_west_pct)
"""min_pct = np.min(total_pct)
max_pct = np.max(total_pct)
rango = max_pct - min_pct
ancho_clase = rango / 5
clases_min = np.arange(min_pct, max_pct + ancho_clase, ancho_clase)"""

#Clases
hist, clases = np.histogram(total_pct, bins=5)
print("Frecuencias de cada categoria: ", hist)
print("Limites de las categorias: ", clases)

#Frecuencia absoluta para cada categoria
frec_cat1 = total_pct[total_pct < clases[1]].size
frec_cat2 = total_pct[(total_pct >= clases[1]) & (total_pct < clases[2])].size
frec_cat3 = total_pct[(total_pct >= clases[2]) & (total_pct < clases[3])].size
frec_cat4 = total_pct[(total_pct >= clases[3]) & (total_pct < clases[4])].size
frec_cat5 = total_pct[total_pct >= clases[4]].size

print(f"Frecuencia absoluta de la clase 1: {frec_cat1}")
print(f"Frecuencia absoluta de la clase 2: {frec_cat2}")
print(f"Frecuencia absoluta de la clase 3: {frec_cat3}")
print(f"Frecuencia absoluta de la clase 4: {frec_cat4}")
print(f"Frecuencia absoluta de la clase 5: {frec_cat5}")

#Frecuencia relativa para cada clase
frec_rel_cat1 = frec_cat1 / total_pct.size
frec_rel_cat2 = frec_cat2 / total_pct.size
frec_rel_cat3 = frec_cat3 / total_pct.size
frec_rel_cat4 = frec_cat4 / total_pct.size
frec_rel_cat5 = frec_cat5 / total_pct.size

print(f"Frecuencia relativa de la clase 1: {frec_rel_cat1}")
print(f"Frecuencia relativa de la clase 2: {frec_rel_cat2}")
print(f"Frecuencia relativa de la clase 3: {frec_rel_cat3}")
print(f"Frecuencia relativa de la clase 4: {frec_rel_cat4}")
print(f"Frecuencia relativa de la clase 5: {frec_rel_cat5}")

#Poligono de frecuencias para el inciso b
bin_centers = (clases[:-1] + clases[1:]) / 2

# Crear el polígono de frecuencias
plt.plot(bin_centers, hist, marker='o')
plt.title('Polígono de Frecuencias de Porcentaje de victorias')
...
# Crear la ojiva menor que
plt.plot(bin_centers, frecuencia_acumulada, marker='o')
plt.title('Ojiva Menor que de Porcentajes de victorias')
plt.xlabel('Porcentaje (%)')
plt.ylabel('Frecuencia Acumulada')
plt.grid(True)
plt.show()